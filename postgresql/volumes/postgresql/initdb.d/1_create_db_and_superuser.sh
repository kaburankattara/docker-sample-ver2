...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
  ...
  -- データベース作成
  CREATE DATABASE "$PROD_DB_NAME" WITH OWNER = "$POSTGRES_USER" ENCODING = 'UTF8' LC_COLLATE = 'C' LC_CTYPE = 'C' CONNECTION LIMIT = -1 TEMPLATE template0;
  ...
  -- データベースへの権限をPOSTGRES_USERロールに付与する
  GRANT ALL ON DATABASE "$PROD_DB_NAME" TO "$POSTGRES_USER";
  GRANT TEMPORARY, CONNECT ON DATABASE "$PROD_DB_NAME" TO PUBLIC;
  ...
  GRANT ALL ON DATABASE "$PROD_DB_NAME" TO "$SUPERUSER_ROLE_NAME";
